...
class TodoStore(ITodoREPO):
    ...
    async def fetch_one_todo(self, todo_id: int) -> Union[TODOSchema, str]:
        try:
            data = await self._collection.find_one({"_id": todo_id}, {"_id": 0})
            if not data:
    ...
    async def fetch_all_todo(self) -> Union[List[TODOSchema], str]:
        try:
            data = self._collection.find({}, {"_id": 0})
            todos = []
    ...
    async def update_todo(self, todo_id: int, todo: TODOSchema) -> str:
        try:
            ...
            if _todo:
                await self._collection.update_one({"_id": todo_id},{"$set": todo.model_dump()})
